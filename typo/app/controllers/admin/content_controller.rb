...
class Admin::ContentController < Admin::BaseController
  ...
  def merge
    ...
    # path: admin/content/merge
    # params => :oid, :mid

    # Ensure that the current_user is an admin. Otherwise, return.
    unless (current_user.admin? && request.post?)
      redirect_to :action => 'index'
      flash[:error] = _("Error: You shall not pass!")
      return
    end

    # Since the current user is admin, find the current_article, and
    # attempt to merge that article with the article to be merged.
    this_art = Article.find_by_id(params[:oid])
    has_merged = this_art && this_art.merge_with(params[:mid])

    # redirect to Edit Index page with appropriate Flash Messages
    if has_merged
      flash[:notice] = _("Success: Articles Merged")
    else
      flash[:error] = _("Error: Cannot merge with non-existent article")
    end
