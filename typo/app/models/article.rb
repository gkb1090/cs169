...
class Article < Content
  ...
  def merge_with(oid)
    other_art = Article.find_by_id oid

    # Other article exists
    if other_art && (oid.to_i != self.id.to_i)

      # Merge body with other articles body
      Article.update(self.id, {:body => self.body + "\n" + other_art.body })

      # Merge Comments
      other_art.comments.each { |c| self.comments << c }

      # Delete the other article and return success
      Article.delete(oid)
      return true
    end
