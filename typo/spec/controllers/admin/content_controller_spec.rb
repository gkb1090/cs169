...
describe Admin::ContentController do
  ...
  describe 'merge action' do
    ...
    before :each do
      Factory(:blog)
      @orig_art = Factory(:article)
      @merge_art = Factory(:article)
    end

    describe 'invalid means of merging' do
      ...
      describe 'merging when user is not an admin' do
        it 'should redirect to Article Index Page ' do
          post :merge, {:oid => @orig_art.id, :mid => @merge_art.id}
          response.should redirect_to(:action => 'index')
        end
      end


      describe 'merge attempt using a GET request' do
        it 'should redirect to Article Index Page ' do
          get :merge
          response.should redirect_to(:action => 'index')
        end
      ...
      after :each do
        ...
        flash[:error].should =~ /Error: You shall not pass!/
      end
    end

    describe 'merging when user is an admin' do
      ...
      it 'should call the merge_with article instance method' do
        Article.should_receive(:find_by_id).with(@orig_art.id).and_return(@orig_art)
        @orig_art.should_receive(:merge_with).once
        post :merge, {:oid => @orig_art.id, :mid => @merge_art.id}
      end

      describe 'the page redirected depending on whether article_id exits or not' do

        it 'should redirect to Index page with Success flash for existent article_id' do
          Article.should_receive(:find_by_id).with(@orig_art.id).and_return(@orig_art)
          @orig_art.should_receive(:merge_with).with(@merge_art.id).and_return(true)
          post :merge, {:oid => @orig_art.id, :mid => @merge_art.id}
          flash[:notice].should_not be_nil
          flash[:notice].should =~ /Success: Articles Merged/
          response.should be_redirect
          response.should redirect_to(:action => 'index')
        end

        it 'should redirect to Index page with Error flash for non-existent merge_article_id' do
          Article.should_receive(:find_by_id).with(@orig_art.id).and_return(@orig_art)
          @orig_art.should_receive(:merge_with).with(@merge_art.id).and_return(false)
          post :merge, {:oid => @orig_art.id, :mid => @merge_art.id}
          flash[:error].should_not be_nil
          flash[:error].should =~ /Error: Cannot merge with non-existent article/
          response.should be_redirect
          response.should redirect_to(:action => 'index')
        end

        it 'should redirect to Index page with Error flash for non-existent original_article_id' do
          Article.should_receive(:find_by_id).with(@orig_art.id).and_return(nil)
