...
describe Article do
  ...
  describe "#merge_with" do
    ...
    before :each do
      @this_art = Factory(:article, :body => 'foo')
      @other_art = Factory(:article, :body => 'bar')
    end

    it 'should merge the content from both articles into the content of the first article' do

      this_id = @this_art.id
      ret_bool = @this_art.merge_with @other_art.id
      ret_bool.should be_true
      Article.find_by_id(this_id).body.should =~ /foo\nbar/

    end

    it 'should merge the comments from both articles' do
      first_comment = Factory(:comment, :article => @this_art)
      second_comment = Factory(:comment, :article => @other_art)
      # Obtain the ids of both comments
      first_comment_id = first_comment.id
      second_comment_id = second_comment.id

      # Obtain the size of both articles comments
      this_size = @this_art.comments.size
      other_size = @other_art.comments.size

      # Both articles should have 1 comment
      this_size.should == 1
      other_size.should == 1
      ...
      # Check that the new size of this articles' comments is the combined length of the above
      @this_art.comments.size.should == (this_size + other_size)

      # Check that this article has both of the original comments
      @this_art.comments.exists?(:id => first_comment_id).should be_true
      @this_art.comments.exists?(:id => second_comment_id).should be_true
    end

    it 'should destroy the other article' do
      other_id = @other_art.id
      Article.find_by_id(other_id).should_not be_nil
      ret_bool = @this_art.merge_with @other_art.id
      ret_bool.should be_true
      Article.find_by_id(other_id).should be_nil
    end

    it 'should have the same author as before' do
      temp_author = @this_art.author
      this_id = @this_art.id
      ret_bool = @this_art.merge_with @other_art.id
      ret_bool.should be_true
      Article.find(this_id).author.should == temp_author
    end

    it 'should return true in happy path' do
      @this_art.merge_with(@other_art.id).should be_true
    end

    it 'should return false if other_article_id is invalid' do
      # Only two articles are defined. Article ID 3 should not exist.
      @this_art.merge_with(3).should_not be_true
    end

    it 'should return false if other_article_id is this_article_id' do
      # Only two articles are defined. Article ID 3 should not exist.
      @this_art.merge_with(@this_art.id).should_not be_true
    end
